...
export const sameAs = (target: Unit) => (unit: Unit) => [...nodeIds(target)].sort().join("") === [...nodeIds(unit)].sort().join(""); // NEW
...
export const correctUnitsShift = (units: readonly Unit[], shift: number) => units?.forEach((unit) => (unit.pos += shift));
