...
import { Node, RelType, Relation } from "../types";
...
const getCoupleNodes = (store: Store, target: Node): readonly Node[] => {
  return [target, getSpouse(store, target?.spouses)].filter((node: unknown): node is Node => Boolean(node)).sort(byGender(store.root?.gender));
};
