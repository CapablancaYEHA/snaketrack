...
export const invalidateSnakeTree = (id: string, category: ECategories) => instance.invalidateQueries({ queryKey: [EQuKeys.FAM_TREE, id, category], exact: true });

// TODO Нужно написать триггеры на удаление из массивов в Breeding и Clutch - ИЛИ ЖЕ
//В Клатч сделана ошибка, что впринципе не можешь удалить змею из активной кладки или плана (из-за foreign column). У меня при удалении получается еще и триггер надо запускать чтобы вычистить из массивов
