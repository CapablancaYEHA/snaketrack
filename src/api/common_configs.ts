import { availMarketStats } from "@/components/common/Market/utils";
...
export const marketAvailSnakes = (userId, c) => ({ t: categoryToBaseTable[c], s: "id, snake_name", f: (b) => b.eq("owner_id", userId).filter("status", "in", `(${availMarketStats.join(",")})`), id: [userId, "limited"] });
